...
import com.juborajsarker.locationfinder.java_class.Location;
import com.juborajsarker.locationfinder.java_class.Result;
import com.juborajsarker.locationfinder.parser.DataParser;
...
public class NearbyMapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    String resultListGson;
    String locationListGson;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        locationListGson = intent.getStringExtra("locationListGson");
        resultListGson = intent.getStringExtra("resultListGson");

        Type typeResult = new TypeToken<List<Result>>(){}.getType();
        Type typeLocation = new TypeToken<List<Location>>(){}.getType();

        locationList = gson.fromJson(locationListGson, typeLocation);
        resultList = gson.fromJson(resultListGson, typeResult);
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        for (int i=0; i < resultList.size(); i++){
            ...
            latitude = locationList.get(i).getLat(); // latitude
            longitude = locationList.get(i).getLng(); // longitude
            ...
            marker2 = mMap.addMarker(new MarkerOptions().position(location2)
                    ...
                    .snippet(resultList.get(i).getVicinity() + " -->" + resultList.get(i).getRating() + "")
                    .alpha(1f));
